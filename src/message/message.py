...
class Message:
    def __init__(self, msg_type, seq_number=0, data=None, timeout=0):
        ...
        if self.data and len(self.data) > DATA_MAX_SIZE:
            logger.error(f"Tamaño máximo de datos es {DATA_MAX_SIZE} bytes, recibido: {len(self.data)}")
            raise ValueError(f"Tamaño máximo de datos es {DATA_MAX_SIZE} bytes")
    ...
    def from_bytes(cls, data):
        ...
        msg_data = data[SEQUENCE_NUMBER_BYTES + 1:] if len(data) > SEQUENCE_NUMBER_BYTES + 1 else b''
